...
// Generic desert location near Phoenix, AZ for the solar farm simulation
const center = { lat: 33.738, lng: -112.186 };

const flightPathCoords = [
  { lat: 33.738, lng: -112.186 },
  { lat: 33.740, lng: -112.182 },
  { lat: 33.742, lng: -112.185 },
  { lat: 33.745, lng: -112.180 },
  { lat: 33.748, lng: -112.183 },
];
...
export default function FlightPlanner() {
  ...
  return (
    <div className="flex-1 flex flex-col lg:flex-row w-full overflow-hidden relative" style={{ height: 'calc(100vh - 64px)' }}>
      ...
      <div className="flex-1 relative bg-surface-dim overflow-hidden flex flex-col">
        ...
        <div className="absolute inset-0 bg-surface-dim overflow-hidden z-0">
          {isLoaded ? (
            ...
            >
              ...
              {/* Start Point */}
              <Marker position={flightPathCoords[0]} icon={{ url: 'http://maps.google.com/mapfiles/ms/icons/green-dot.png' }} />
              {/* End Point */}
              <Marker position={flightPathCoords[flightPathCoords.length - 1]} icon={{ url: 'http://maps.google.com/mapfiles/ms/icons/red-dot.png' }} />
            </GoogleMap>
          ) : (
        ...
        <div className="relative z-10 p-sm md:p-md flex justify-between items-start pointer-events-none">
          <div className="pointer-events-auto bg-surface-container-lowest/90 backdrop-blur-md rounded-lg shadow-sm border border-outline-variant p-base inline-flex flex-col gap-base">
            ...
            <div className="h-[1px] w-full bg-outline-variant/50"></div>
            <button className="p-xs hover:bg-surface-container-high rounded-md transition-colors text-on-surface-variant cursor-pointer" title="Center on Farm">
          ...
          <div className="pointer-events-auto bg-surface-container-lowest/90 backdrop-blur-md rounded-full shadow-sm border border-outline-variant px-sm py-xs flex items-center gap-sm">
            <div className="flex items-center gap-xs text-primary">
              ...
              <span className="font-label-md text-label-md">Path: 4.2km</span>
            </div>
            <div className="w-[1px] h-4 bg-outline-variant"></div>
            <div className="flex items-center gap-xs text-on-surface-variant">
              <span className="material-symbols-outlined text-[18px]">location_on</span>
              <span className="font-label-md text-label-md">5 Waypoints</span>
            </div>
          </div>
        </div>
        
        {/* Bottom floating action (Mobile Primary) */}
        <div className="mt-auto p-margin-mobile relative z-10 lg:hidden w-full flex justify-center pointer-events-none">
      ...
      <aside className="w-full lg:w-[400px] bg-surface-container-lowest border-l border-outline-variant flex flex-col h-full overflow-y-auto lg:h-[calc(100vh-64px)] z-20 shadow-[-4px_0_12px_rgba(30,41,59,0.02)]">
        {/* Sidebar Header */}
        ...
        <div className="p-md flex flex-col gap-lg flex-1">
          ...
          <div className="bg-surface-container-low border border-outline-variant rounded-lg p-sm flex items-start gap-sm">
            <div className="p-xs bg-surface-container-highest rounded-full text-on-surface-variant flex-shrink-0 flex">
